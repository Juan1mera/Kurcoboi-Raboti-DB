...
function getDetallesVenta($id_venta) {
    try {
        $conn = getDBConnection();
        $sql = "SELECT dv.*, v.fecha AS fecha_venta, c.nombre AS nombre_cliente, p.nombre AS nombre_producto
                FROM Detalle_Ventas dv
        ...
        throw new Exception("Error al obtener detalles de venta: " . $e->getMessage());
    }
}
...
function deleteDetallesByVenta($id_venta) {
    try {
        $conn = getDBConnection();
        $sql = "DELETE FROM Detalle_Ventas WHERE id_venta = :id_venta";
        $stmt = $conn->prepare($sql);
        $stmt->execute([':id_venta' => $id_venta]);
        return $stmt->rowCount() > 0;
    } catch (PDOException $e) {
        throw new Exception("Error al eliminar detalles de venta: " . $e->getMessage());
    }
}
?>